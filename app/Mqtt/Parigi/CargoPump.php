<?php

declare(strict_types=1);
/**
 * This file is part of Hyperf.
 *
 * @link     https://www.hyperf.io
 * @document https://hyperf.wiki
 * @contact  group@hyperf.io
 * @license  https://github.com/hyperf/hyperf/blob/master/LICENSE
 */
namespace App\Mqtt\Parigi;

use Carbon\Carbon;
use Hyperf\Utils\Codec\Json;
use Hyperf\Utils\Str;

class CargoPump
{
    protected string $message;

    protected $mappArray = [
    ];

    public function __construct(string $message)
    {
        $this->message = $message;
    }

    public function extract()
    {
        $data = Json::decode($this->message);

        return [
            'cargo' => [
                'cargo_pump_timestamp' => Carbon::now()->format('Y-m-d H:i:s'),
                'cargo_pump1_run' => (bool) $data['cargo_pump1_run'],
                'cargo_pump2_run' => (bool) $data['cargo_pump2_run'],
                'cargo_pump3_run' => (bool) $data['cargo_pump3_run'],
                'ballast_pump1_run' => (bool) $data['ballast_pump1_run'],
                'ballast_pump2_run' => (bool) $data['ballast_pump2_run'],
                'stripping_pump_run' => (bool) $data['stripping_pump_run'],
                'vacuum_pump1_run' => (bool) $data['vacuum_pump1_run'],
                'vacuum_pump2_run' => (bool) $data['vacuum_pump2_run'],
                'tank_cleaning_pump_run' => (bool) $data['tank_cleaning_pump_run'],
            ],
        ];
    }

    public function arrayToSnake(): array
    {
        $snake = [];
        foreach ($this->mappArray as $in => $val) {
            if (is_null($val)) {
                continue;
            }
            $key = Str::snake(strtolower($val));
            $snake[$key] = $val;
        }
        return $snake;
    }
}
