 ...
 */
namespace Smartship\Gasparta2\Process;
...
#[Process(name: 'smartship-gasparta2', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('gasparta2.fleet_id');
        $config = config('gasparta2.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('gasparta2.logger');
        ...
        $topics = config('gasparta2.topics', []);
    ...
    {
        return (bool) config('gasparta2.enable', false);
    }
}
