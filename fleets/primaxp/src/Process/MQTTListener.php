 ...
 */
namespace Smartship\Primaxp\Process;
...
use Smartship\Primaxp\Event\MQTTReceived;

use function Hyperf\Config\config;

#[Process(name: 'smartship-primaxp', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('primaxp.fleet_id');
        $config = config('primaxp.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('primaxp.logger');
    ...
    {
        return (bool) config('primaxp.enable', false);
    }
}
