 ...
 */
namespace Smartship\Bima\Process;
...
#[Process(name: 'smartship-bima', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('bima.fleet_id');
        $config = config('bima.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('bima.logger');
    ...
    {
        return (bool) config('bima.enable', false);
    }
}
