 ...
 */
namespace Smartship\Plaju\Process;
...
#[Process(name: 'smartship-plaju', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('plaju.fleet_id');
        $config = config('plaju.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('plaju.logger');
    ...
    {
        return (bool) config('plaju.enable', false);
    }
}
