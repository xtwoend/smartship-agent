 ...
 */
namespace Smartship\Arafura\Process;
...
class MQTTListener extends AbstractProcess
{
    ...
    {
        $fleet_id = config('arafura.fleet_id');
        $config = config('arafura.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('arafura.logger');
    ...
    {
        return (bool) config('arafura.enable', false);
    }
}
