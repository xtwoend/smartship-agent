 ...
 */
namespace Smartship\Klasogun\Process;
...
#[Process(name: 'smartship-klasogun', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('klasogun.fleet_id');
        $config = config('klasogun.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('klasogun.logger');
    ...
    {
        return (bool) config('klasogun.enable', false);
    }
}
