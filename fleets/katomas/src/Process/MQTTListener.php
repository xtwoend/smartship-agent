 ...
 */
namespace Smartship\Katomas\Process;
...
#[Process(name: 'smartship-katomas', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('katomas.fleet_id');
        $config = config('katomas.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('katomas.logger');
    ...
    {
        return (bool) config('katomas.enable', false);
    }
}
