 ...
 */
namespace Smartship\SangaSanga\Process;
...
#[Process(name: 'smartship-sanga', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('sanga.fleet_id');
        $config = config('sanga.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('sanga.logger');
    ...
    {
        return (bool) config('sanga.enable', false);
    }
}
