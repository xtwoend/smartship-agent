 ...
 */
namespace Smartship\Pagaden\Process;
...
use Smartship\Pagaden\Event\MQTTReceived;

use function Hyperf\Config\config;

#[Process(name: 'smartship-pagaden', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('pagaden.fleet_id');
        $config = config('pagaden.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('pagaden.logger');
    ...
    {
        return (bool) config('pagaden.enable', false);
    }
}
