 ...
 */
namespace Smartship\Taurus\Process;
...
#[Process(name: 'smartship-taurus', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('taurus.fleet_id');
        $config = config('taurus.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('taurus.logger');
    ...
    {
        return (bool) config('taurus.enable', false);
    }
}
