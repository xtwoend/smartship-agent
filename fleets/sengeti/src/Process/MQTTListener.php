 ...
 */
namespace Smartship\Sengeti\Process;
...
#[Process(name: 'smartship-sengeti', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('sengeti.fleet_id');
        $config = config('sengeti.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('sengeti.logger');
    ...
    {
        return (bool) config('sengeti.enable', false);
    }
}
