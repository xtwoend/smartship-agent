 ...
 */
namespace Smartship\Mauhau\Process;
...
use Smartship\Mauhau\Event\MQTTReceived;

use function Hyperf\Config\config;

#[Process(name: 'smartship-mauhau', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('mauhau.fleet_id');
        $config = config('mauhau.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('mauhau.logger');
    ...
    {
        return (bool) config('mauhau.enable', false);
    }
}
