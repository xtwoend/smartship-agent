 ...
 */
namespace Smartship\Ketaling\Process;
...
use Smartship\Ketaling\Event\MQTTReceived;

use function Hyperf\Config\config;

#[Process(name: 'smartship-ketaling', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('ketaling.fleet_id');
        $config = config('ketaling.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('ketaling.logger');
    ...
    {
        return (bool) config('ketaling.enable', false);
    }
}
