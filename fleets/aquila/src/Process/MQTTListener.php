 ...
 */
namespace Smartship\Aquila\Process;
...
class MQTTListener extends AbstractProcess
{
    ...
    {
        $fleet_id = config('aquila.fleet_id');
        $config = config('aquila.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('aquila.logger');
    ...
    {
        return (bool) config('aquila.enable', false);
    }
}
