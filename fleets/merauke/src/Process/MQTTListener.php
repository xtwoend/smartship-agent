 ...
 */
namespace Smartship\Merauke\Process;
...
class MQTTListener extends AbstractProcess
{
    ...
    {
        $fleet_id = config('merauke.fleet_id');
        $config = config('merauke.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('merauke.logger');
    ...
    {
        return (bool) config('merauke.enable', false);
    }
}
