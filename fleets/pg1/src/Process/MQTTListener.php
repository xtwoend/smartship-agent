 ...
 */
namespace Smartship\Pg1\Process;
...
class MQTTListener extends AbstractProcess
{
    ...
    {
        $fleet_id = config('pg1.fleet_id');
        $config = config('pg1.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('pg1.logger');
    ...
    {
        return (bool) config('pg1.enable', false);
    }
}
