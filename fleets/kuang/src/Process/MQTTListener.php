 ...
 */
namespace Smartship\Kuang\Process;
...
#[Process(name: 'smartship-kuang', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('kuang.fleet_id');
        $config = config('kuang.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('kuang.logger');
    ...
    {
        return (bool) config('kuang.enable', false);
    }
}
