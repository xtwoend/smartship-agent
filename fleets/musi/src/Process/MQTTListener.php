 ...
 */
namespace Smartship\Musi\Process;
...
class MQTTListener extends AbstractProcess
{
    ...
    {
        $fleet_id = config('musi.fleet_id');
        $config = config('musi.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('musi.logger');
    ...
    {
        return (bool) config('musi.enable', false);
    }
}
