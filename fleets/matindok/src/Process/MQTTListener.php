 ...
 */
namespace Smartship\Matindok\Process;
...
#[Process(name: 'smartship-matindok', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('matindok.fleet_id');
        $config = config('matindok.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('matindok.logger');
    ...
    {
        return (bool) config('matindok.enable', false);
    }
}
