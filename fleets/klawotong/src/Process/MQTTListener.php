 ...
 */
namespace Smartship\Klawotong\Process;
...
#[Process(name: 'smartship-klawotong', redirectStdinStdout: false, pipeType: 1, nums: 1, enableCoroutine: false)]
class MQTTListener extends AbstractProcess
{
    public function handle(): void
    {
        $fleet_id = config('klawotong.fleet_id');
        $config = config('klawotong.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('klawotong.logger');
        ...
        $topics = config('klawotong.topics', []);
    ...
    {
        return (bool) config('klawotong.enable', false);
    }
}
