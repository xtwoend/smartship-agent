 ...
 */
namespace Smartship\Krasak\Process;
...
class MQTTListener extends AbstractProcess
{
    ...
    {
        $fleet_id = config('krasak.fleet_id');
        $config = config('krasak.mqtt_connection');
        $clientId = Str::random(10);
        $classLogger = config('krasak.logger');
    ...
    {
        return (bool) config('krasak.enable', false);
    }
}
